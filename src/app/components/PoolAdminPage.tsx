...
export function PoolAdminPage() {
  ...
  async function handleCreateOnchain() {
    ...
    try {
      ...
      tx.moveCall({
        target: `${form.packageId}::pool_manager::create_pool`,
      ...
      const saved = await saveToBackend({ ...form, poolObjectId }, null);
      setNotice(`Пул создан on-chain и сохранен в БД: ${shortId(saved.pool_object_id)}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(null);
  ...
  async function handleUpdateOnchain() {
    ...
    try {
      ...
      tx.setSender(account!.address);
      tx.moveCall({
        target: `${form.packageId}::pool_manager::update_pool`,
        typeArguments: [form.coinType],
        arguments: [
          tx.object(form.poolAdminCapId || SUI_FUND_CONFIG.poolAdminCapId),
          tx.object(form.poolRegistryId || SUI_FUND_CONFIG.poolRegistryId),
