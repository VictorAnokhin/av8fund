...
function isDeskNoteArticle(article: NewsArticle): boolean {
  const value = (article.htmlkeys || '').toLowerCase();
  return (article.hot || 0) === 1 || value.includes('desk-note') || value.includes('desk note') || value.includes('desknote');
}
...
export function HorizonAnalyticsPanel() {
  ...
  return (
    <div className="ce-glass-slab rounded-[2rem] p-5">
      ...
      <div className="h-[190px] rounded-[1.75rem] border border-white/8 bg-[linear-gradient(180deg,rgba(15,23,42,0.86)_0%,rgba(3,7,18,0.96)_100%)] p-2 sm:h-[220px] sm:p-3 md:h-[250px] lg:h-[280px]">
...
export function Analytics({ project }: AnalyticsProps) {
  ...
  const importantNewsRef = React.useRef<HTMLDivElement | null>(null);
  ...
  const twitterUrl = project.twitter
    ...
    : '';
  const canScrollImportantNews = deskNotes.length > 4;
  const scrollImportantNews = React.useCallback((direction: 'prev' | 'next') => {
    const node = importantNewsRef.current;
    if (!node) {
      return;
    }

    const step = Math.max(node.clientWidth * 0.9, 320);
    node.scrollBy({
  ...
  React.useEffect(() => {
    ...
    async function loadDeskNotes() {
      try {
        ...
        const preferred = news.filter(isDeskNoteArticle);
        const fallback = news.filter((item) => !preferred.some((preferredItem) => preferredItem.id === item.id));
        const nextItems = [...preferred, ...fallback].slice(0, 5);
  ...
  return (
    <section id="analytics" className="relative overflow-hidden border-t border-white/[0.06] bg-[linear-gradient(180deg,rgba(6,9,16,0.55)_0%,rgba(7,11,22,0.85)_100%)] py-16">
      ...
      <div className="relative mx-auto max-w-7xl space-y-6 px-6">
        <div className="ce-glass-slab rounded-[2rem] p-5">
          <div className="mb-4 flex items-center justify-between gap-4">
            <div>
              <div className="text-xs uppercase tracking-[0.18em] text-slate-400">Market watch</div>
              <div className="mt-2 text-2xl font-semibold text-white">Live token board</div>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {marketTokens.map((token) => {
              ...
              return (
                <div
                  key={token.symbol}
                  className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/8 bg-slate-950/60 px-4 py-4 text-sm shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]"
                >
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1 text-right tabular-nums">
                    <div className="font-mono text-sm text-slate-200">{token.price}</div>
                    <div className={`font-mono text-xs leading-none ${positive ? 'text-emerald-300' : 'text-rose-300'}`}>
        ...
        <div className="ce-glass-slab rounded-[2rem] p-5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-semibold text-white">Важные новости</div>
              </div>
            </div>
            {canScrollImportantNews ? (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => scrollImportantNews('prev')}
                  className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:border-teal-300/30 hover:text-teal-200"
                ...
                <button
                  type="button"
                  onClick={() => scrollImportantNews('next')}
                  className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:border-teal-300/30 hover:text-teal-200"
          ...
          {deskNotesLoading ? (
            <p className="rounded-2xl border border-white/8 bg-slate-950/55 px-4 py-4 text-sm leading-6 text-slate-300">
              Loading news...
            </p>
          ) : deskNotes.length > 0 ? (
            <div
              ref={importantNewsRef}
              className="flex snap-x gap-3 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:thin] [scrollbar-color:rgba(45,212,191,0.45)_rgba(15,23,42,0.55)]"
            >
              {deskNotes.map((item) => (
                <a
                  key={item.id}
                  href={getArticlePath(item.id)}
                  className="group flex min-h-[190px] w-[82%] shrink-0 snap-start flex-col rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-4 transition hover:border-teal-300/35 hover:bg-slate-900/80 sm:w-[46%] lg:w-[calc((100%_-_2.25rem)_/_4)]"
                >
                  <h3 className="text-lg font-semibold leading-snug text-white transition group-hover:text-teal-200">
                    {item.title}
                  </h3>
                  {item.excerpt ? (
                  ...
                  <div className="mt-auto inline-flex items-center gap-2 pt-4 text-sm font-semibold text-teal-300">
                    Читать статью
